...
## CI/Container Assumptions

- The Docker/Compose path is designed to run entirely inside the image, not via bind-mounting the repo into the container.
- [`docker-compose.yml`](./docker-compose.yml) defines the `workspace` service used by CI and local containerized builds.
- If a containerized build can run `gforth` but fails at the packaging stage, the usual missing dependency is `avr-objcopy`.
