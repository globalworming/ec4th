...
\ empty loop for sle88 simulation
: ms 150 * 0 ?DO LOOP ;
: blank  bl fill ;
...
'J	value left-key		\ customize if you don't like them
'K	value rot-key
'L	value right-key
bl	value drop-key
'P	value pause-key
...
: randomize	1234 seed ! ( time&date + + + + + seed ! ) ;

1 cells 4 = [IF]
$10450405 Constant generator \ FIXME: Zahl zu groß für 16 bittigen AVR
...
: random	\ max --- n ; return random number < max
		seed @ 13 * 1+ [ hex ] 07FFF [ decimal ] and
		dup seed !  swap mod ;
[THEN]
...
: update-score	\ --- ; display current score
		38 16 at-xy score @ . \ 3 .r \ FIXME: Wieder 3 .r <--schauen warums nich geht
