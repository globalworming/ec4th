...
CREATE BranchTable 15 cells allot
here 3 cells -
ACONSTANT MaxTable
...
: next-head ( addr1 -- addr2 ) \ gforth
    ...
    here >r
    BEGIN dup head? IF rdrop EXIT THEN cell+ dup r@ u>= UNTIL
    drop r> ;
\ old version, I don't prefer u+do because I want to be able to
\ cross-compile see.fs
\    here swap u+do
\	i head?
\	if
\	    i unloop exit
...
: xt-see ( xt -- ) \ gforth
    ...
    dup docon: = IF drop seecon EXIT THEN
    dup docol: = IF drop seecol EXIT THEN
    dup dovar: = IF drop seevar EXIT THEN
[ [IFDEF] douser: ]
    dup douser: = IF drop seeuser EXIT THEN
[ [THEN] ]
[ [IFDEF] dodefer: ]
    dup dodefer: = IF drop seedefer EXIT THEN
[ [THEN] ]
[ [IFDEF] dofield: ]
   dup dofield: = IF drop seefield EXIT THEN
[ [THEN] ]
   over over = IF drop seecode EXIT THEN \ direct threaded code words
   over >body over = IF drop seecode EXIT THEN \ indirect threaded code words
