...
\        false swap
\        BEGIN @ dup
\        WHILE \ dup name>string type space
\              dup name>int
...
: (look) ( xt startlfa -- lfa flag )
  BEGIN @ dup WHILE dup name>int ( S xt lfa xt2 ) 2 pick =
        IF nip -1 EXIT THEN
  REPEAT nip 0 ;
...
[IF]
: look
    dup [ unlock rom-dictionary borders lock swap ] 
    aliteral aliteral within
    IF
	>head-noprim dup ??? <>
    ELSE
        dup xtprim?
        IF   \ search only primitives
             ['] xtprim? >head-noprim (look)
        ELSE \ search all
 	     forth-wordlist wordlist-id (look)
        THEN
    THEN ;
[ELSE]
: look ( cfa -- lfa flag )
