...
\ Optimized value for romable code that compiles word header to rom
\ and points to data field in ram. this is optional for rom enabled
\ systems. If false (default) the whole word header will be compiled
\ to ram (via BuildSmart) in systems with rom.
false DefaultValue rom-value
...
: dump-region ( region -- )
\G dump region from start to dp
  dup >rdp @ over >rstart @ - >r
  >rmem @ r> dump ;
...
\ compiles complete word in rom or ram
Builder (Value)
Build:  ( n -- ) ;Build
